...
class JdTdudfp:
    ...
    async def _get(self):
        ...
        try:
            ...
            #找到购物车
            # await page.waitFor(".cw-icon")
            # .cate_menu_lk是一个a标签，理论上可以直接触发click事件
            # 点击事件会打开一个新的tab页，但是browser.pages()无法获取新打开的tab页，导致无法引用新打开的page对象
            # 所以获取href，使用goto跳转的方式
            # 下面类似goto写法都是这个原因
            # a_href = await page.querySelectorAllEval(".cw-icon a", "(elements) => elements[0].href")
            # logger.info(a_href)
            # await page.goto(a_href)
            #进入购物车找到里面的商品链接
            await page.waitFor(".p-msg .p-name a")
            logger.info("page_title：【%s】, page_url：【%s】" % (await page.title(), page.url))
            a_href = await page.querySelectorAllEval(".p-msg .p-name a", "(elements) => elements[0].href")
            await page.goto(a_href)
            #进入商品页加入购物车
            ...
            logger.info("page_title：【%s】, page_url：【%s】" % (await page.title(), page.url))
            await page.waitFor("#eid")
            await page.waitFor("#fp")
            eid = await page.querySelectorAllEval("#eid", "(elements) => elements[0].value")
            fp = await page.querySelectorAllEval("#fp", "(elements) => elements[0].value")
